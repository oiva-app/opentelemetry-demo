...
if (typeof window !== 'undefined' && window.location) {
  ...
  protocol = window.location.protocol.slice(0, -1) // Remove trailing ':' OR ELSE
}

export default function imageLoader({ src, width, quality }) {
  // We pass down the optimisation request to the
  // image-provider service here, without this, nextJs would try
  // to use internal optimiser which is not working with the external image-provider.
  return `${protocol}://${host}:${port}/${src}?w=${width}&q=${quality || 75}`;
}
